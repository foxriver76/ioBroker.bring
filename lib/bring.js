...
class Bring {
    ...
    async saveItem(listUuid, itemName, specification) {
        let data;
        try {
            data = await request.put(`${this.url}bringlists/${listUuid}`, {headers: this.putHeaders, body: `&purchase=${itemName}&recently=&specification=${specification}&remove=&sender=null`});
    ...
    async removeItem(listUuid, itemName) {
        let data;
        try {
            data = await request.put(`${this.url}bringlists/${listUuid}`, {headers: this.putHeaders, body: `&purchase=&recently=&specification=&remove=${itemName}&sender=null`});
            return Promise.resolve(data);
        } catch (e) {
    ...
    async moveToRecentList(listUuid, itemName) {
        let data;
        try {
            data = await request.put(`${this.url}bringlists/${listUuid}`, {headers: this.putHeaders, body: `&purchase=&recently=${itemName}&specification=&remove=&&sender=null`});
            return Promise.resolve(data);
        } catch (e) {
            return Promise.reject(`Cannot remove item ${itemName} from ${listUuid}: ${e}`);
        } // endCatch
